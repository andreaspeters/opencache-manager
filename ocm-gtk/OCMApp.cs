...
namespace ocmgtk
{
	...
	/* OLD APP
			...
			if (args != null)
				if (args.Length > 0)
					m_file = args[0];
			//System.Console.WriteLine("Path is " + "/usr/local/opencachemanager/share/locale");
			// Set the localeDirectory right both for developement or for installed versions
			String localeDirectory = "/usr/local/opencachemanager/share/locale";
			if (localeDirectory.Contains("@" + "expanded_datadir" + "@")) {
				localeDirectory = "./locale";
			}
			Mono.Unix.Catalog.Init ("opencachemanager", localeDirectory);
			//Mono.Unix.Catalog.Init ("opencachemanager", "./locale");
			//Mono.Unix.Catalog.Init ("opencachemanager", "/usr/local/opencachemanager/share/locale");
